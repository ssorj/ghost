...
class GhostCommand(_commandant.Command):
    ...
    def clone_command(self):
        ...
        _plano.run("git clone git@github.com:{}/{}.git".format(self.args.user, self.args.repo_name))
    ...
    def init_command(self):
        ...
        repo_name = _plano.get_base_name(self.args.repo_dir)

        with _plano.working_dir(self.args.repo_dir):
            _plano.run("git init")
            _plano.run("git add .")
            _plano.run("git commit -m \"Initial commit\"")
            _plano.run("git remote add origin git@github.com:{}/{}.git".format(self.args.user, repo_name))
    ...
    def status_command(self):
        ...
        for repo_dir in self.args.repo_dir:
            ...
            with _plano.working_dir(repo_dir):
                output = _plano.call("git status -sb")
